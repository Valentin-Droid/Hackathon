...
app.post("/statsaddcards", (req, res) => {
  ...
  const query =
    "UPDATE site_stats SET cards_generate = cards_generate + 1 WHERE user = ?";
...
app.get("/stats", (req, res) => {
  const query =
    "SELECT pdf_read, cards_generate, adv_notes FROM site_stats WHERE user = ?";
  ...
  db.query(query, ["local"], (err, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "Aucune donnée trouvée pour l'utilisateur 'local'" });
    }
